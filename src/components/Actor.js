    ...
    return (
        <>
            ...
            <div className={"actor-main"}>
                {isLoading ? <Spinner/> : <div className="actor-detail-div">
                    ...
                    <div className="actor-detail-part">
                        ...
                        {actorFilms.map(film=><p key={btoa(film.url)} onClick={()=>{openModal(film.url)}} className="actor-movie-item">{film.title}</p>)}
                    </div>
                </div>}
            </div>
        </>
