...
export default function FilmCard({movie, url}) {
  ...
  return (
    <div className="filmcard-main">
      {isLoading && <Spinner />}
      {!isLoading && data && <>
        <Link to={`/films/${btoa(data.url)}`} className="filmcard-title-link"><h2 className="filmcard-title">{data.title} E.{data.episode_id}</h2></Link>
        <hr/>
        <div className="filmcard-detail-div"><p>{data.opening_crawl.split('\n\r').splice(0, 1)[0]}</p></div>
        <hr/>
