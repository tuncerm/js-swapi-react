  ...
  useEffect(() => {
    const fetchCharacter = async () => {
      try {
        ...
        c.planet = response.homeworld
          ? (await getData(response.homeworld)).name
          : "";
        c.species = [];
        c.vehicles = [];
        c.starships = [];
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="character-detail-div">
            ...
            <p>
              Vehicles:{" "}
              {(character.vehicles && character.vehicles.join(", ")) || "N/A"}
            </p>
            <p>
              Starhips:{" "}
              {(character.starships && character.starships.join(", ")) || "N/A"}
            </p>
          </div>
        </>
      )}
